...
$mssqlIsoUrl = $Env:MSSQL2017INSTALL_ISO_URL;
$mssqlCuUrl = $Env:MSSQL2017INSTALL_CU_URL;
# (C) URL to the CU manual patch (https://github.com/microsoft/mssql-docker/issues/540)
$mssqlCuFixUrl = $Env:MSSQL2017INSTALL_CUFIX_URL;
...
SbsDownloadFile -Url $mssqlCuUrl  -Path "C:\MSSQLUPDATES\SQLServer2017-CU.exe";

# Download SQL Server ISO and extract
SbsDownloadFile -Url $mssqlIsoUrl -Path "C:\SQLServer2017-x64-ENU-Dev.iso.iso";
New-Item -Path C:\SQLServerISO -ItemType Directory;
7z x C:\SQLServer2017-x64-ENU-Dev.iso.iso -oC:\SQLServerISO;
Remove-Item -Path C:\SQLServer2017-x64-ENU-Dev.iso.iso -Force;
